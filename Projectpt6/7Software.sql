...
SPOOL C:\Users\Shaquille\Projectpt6\PT6.txt
...
SELECT consultant_skill.c_id "Consultant ID", skill.skill_description "Skill"
FROM consultant_skill, skill
WHERE consultant_skill.skill_id = skill.skill_id AND c_id = 100 ;

-------B----------

SELECT consultant_skill.c_id "Consultant ID",skill.skill_description "Skill",consultant_skill.certification "Certified Y/N"
FROM consultant_skill, skill
WHERE consultant_skill.skill_id = skill.skill_id AND c_id = 102 AND certification = 'Y' ;

------C-----------
SELECT project_consultant.c_id "Consultant ID", project.project_name "Project Name"
FROM project, project_consultant
WHERE project_consultant.p_id = project.p_id AND project_consultant.c_id = 100 ;

------D----------

SELECT skill.skill_description "Skill", consultant.c_first "First Name", consultant.c_last "Last Name"
FROM consultant, consultant_skill, skill
WHERE consultant.c_id = consultant_skill.c_id AND consultant_skill.skill_id = skill.skill_id AND certification = 'Y';

-- SELECT consultant_skill.skill_id
...
SELECT consultant.c_id "Consultant ID",consultant.c_first "First Name", consultant.c_last "Last Name",project.project_name "Project Name"
FROM consultant, project, project_consultant
WHERE consultant.c_id = project_consultant.c_id AND project.p_id = project_consultant.p_id AND consultant.c_id = 100 ;



-------F-----------
SELECT DISTINCT project.project_name "Project Name",consultant.c_first "Manager First Name",consultant.c_last "Manager Last Name"
FROM project,consultant,project_consultant
WHERE project_consultant.p_id = project.p_id AND project.mgr_id = consultant.c_id ;


--------G--------- 
SELECT progp.project_name "Project Name", prog.project_name "Parent Project"
FROM project prog, project progp
WHERE progp.parent_p_id = prog.p_id ;
...
SELECT progp.project_name "Project Name", prog.project_name "Parent Project"
FROM project prog, project progp
WHERE prog.p_id (+) = progp.parent_p_id;
